...
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    if (file.fieldname === 'roomImage') {
      ...
      cb(null, path.join(__dirname, '../../public/hotelView/'));
    } else {
...
export const createHotel = (req: Request, res: Response): void => {
  upload(req, res, (err: any) => {
    ...
    try {
      ...
      const images = (req.files && (req.files as { [fieldname: string]: Express.Multer.File[] })['images'])
        ? (req.files as { [fieldname: string]: Express.Multer.File[] })['images'].map((file) => file.filename)
        : [];
